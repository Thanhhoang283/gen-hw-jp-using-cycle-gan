...
def warp_image(img, random_state=None, **kwargs):
    ...
    interval = np.random.randint(height/4, height/2)
    std = np.random.uniform(2.0, 3.5)

    w_mesh_interval = kwargs.get('w_mesh_interval', interval)
    w_mesh_std = kwargs.get('w_mesh_std', std)

    h_mesh_interval = kwargs.get('h_mesh_interval', interval)
    h_mesh_std = kwargs.get('h_mesh_std', std)

    interpolation_method = kwargs.get('interpolation', 'cubic')
