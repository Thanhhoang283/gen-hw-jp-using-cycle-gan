...
def main(path, source_path, ratioA, ratioB):
    ...
    datafolder = os.path.join(os.path.normpath(directory + os.sep + os.pardir), 'datasets')
    if not os.path.exists(datafolder):
        os.makedirs(datafolder)
    ...
    for x in np.arange(len(arr)):
        ...
        if arr[x]<=ntrainA and arr[x]<=ntrainB:
            scipy.misc.imsave(os.path.join(trainA_path, ch + '.png'), bitmap)
            scipy.misc.imsave(os.path.join(trainB_path, ch + '.png'), source_img)
        elif arr[x]>ntrainA and arr[x]<=ntrainB:
            scipy.misc.imsave(os.path.join(testA_path, ch + '.png'), bitmap)
            scipy.misc.imsave(os.path.join(trainB_path, ch + '.png'), source_img)
        elif arr[x]<=ntrainA and arr[x]>ntrainB:
            scipy.misc.imsave(os.path.join(trainA_path, ch + '.png'), bitmap)
            scipy.misc.imsave(os.path.join(testB_path, ch + '.png'), source_img)
        else:
            scipy.misc.imsave(os.path.join(testA_path, ch + '.png'), bitmap)
            scipy.misc.imsave(os.path.join(testB_path, ch + '.png'), source_img)
