...
-- paths.mkdir(web_dir)
-- local try = paths.concat(opt.results_dir)
-- print("************************", try)
-- local image_dir = paths.concat(opt.results_dir)
...
for n = 1, math.floor(opt.how_many) do
  ...
  for i,visual in ipairs(visuals) do
    ...
    if visual.label == "fake_B" then
      visualizer.save_images(visual.img, paths.concat(image_dir), {string.gsub(cur_pathsA[1],'.jpg','.png')}, s1, s2)
    end
    -- print(visual)
  end
