...
def main(line_th, text, source_font, model, image_dir, label_dir):
    path = os.getcwd()
    ...
    os.system(" DATA_ROOT={} name={} phase=test \
        results_dir={} th test.lua".format(printed_folder, '1252-c-dense5', hw_folder))
    
    list_imgs = [cv2.imread(os.path.join(hw_folder, 
        str(index) + '.png'), 0) for index in range(len(text.decode('utf-8')))]

    for index, ch in enumerate(text.decode('utf-8')):
        cv2.imwrite(os.path.join(image_dir, '{}.{}_{}.png'.format(line_th, index, ch)), list_imgs[index])
        otsu_threshold = threshold_otsu(list_imgs[index])
        binary_image = (list_imgs[index] < otsu_threshold) > 0

        x, y, width, height = cv2.boundingRect(255-((1-binary_image)*255).astype('uint8'))
        # color_image = cv2.cvtColor(list_imgs[index], cv2.COLOR_GRAY2BGR)
        # cv2.rectangle(color_image, (x,y), (x+width, y+height), thickness=3, color=(255,0,0))
        # plt.imshow(color_image)
        # plt.show()
    
        with open(os.path.join(label_dir, "gt_{}.{}_{}.txt".format(line_th, index, ch)), "w") as text_file:
            text_file.write("{}, {}, {}, {}, {}, {}, {}, {}".format(x, y, x+width, y, x+width, y+height, x, y+height))

    # combine_images(list_imgs, line_th, save_directory)
...
if __name__ == '__main__':
    ...
    #     save_directory = os.path.join(folder, 'training_data')
    ...
    #     model = models[np.random.randint(3)]
    #     # main(i, line, args.font, model, image_dir, label_dir)
    #     try:
    ...
    for key, value in data.items():
        new_dictionary[key+'.png'] = value

    for key,value in new_dictionary.items():
        print(key)
    print(len(new_dictionary.items()))

    with open('new_hw_labels.json', 'w') as fp:
